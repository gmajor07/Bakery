...
import 'new_product_adjustment_screen.dart';
...
class ProductAdjustmentsScreen extends ConsumerStatefulWidget {
  const ProductAdjustmentsScreen({super.key});

  @override
  ConsumerState<ProductAdjustmentsScreen> createState() =>
      _ProductAdjustmentsScreenState();
}

class _ProductAdjustmentsScreenState
    extends ConsumerState<ProductAdjustmentsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final adjustmentsAsync = ref.watch(productAdjustmentsProvider);
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () {
              ref.invalidate(productAdjustmentsProvider);
            },
            tooltip: 'Refresh',
          ),
          if (hasFilters)
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => const NewProductAdjustmentScreen(),
            ),
          );
        },
        icon: const Icon(Icons.add),
        label: const Text('Product Adjustment'),
      ),
      body: Column(
        children: [
          Padding(
          ...
          Expanded(
            child: adjustmentsAsync.when(
              data: (adjustments) {
                ...
                // ⭐️ IMPLEMENTED: RefreshIndicator for Pull-to-Refresh
                return RefreshIndicator(
                  onRefresh: () async {
                    // Refreshes the provider to fetch latest data
                    return ref.refresh(productAdjustmentsProvider.future);
                  },
                  child: ListView.builder(
                    padding: const EdgeInsets.symmetric(horizontal: 16),
                    // AlwaysScrollable ensures we can pull even if list is short
                    physics: const AlwaysScrollableScrollPhysics(),
                    itemCount: adjustments.length,
                    itemBuilder: (context, index) {
                      return ProductAdjustmentTile(
        ...
        children: [
          ...
          Text(
            hasFilters
                ...
                : 'No product adjustments recorded yet.',
...
class ProductAdjustmentTile extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final amount = adjustment.amount;
    ...
    final formattedDate = DateFormat('dd-MM-yyyy').format(adjustment.createdAt);

    final reasonText =
    adjustment.reason.isNotEmpty ? adjustment.reason : 'No reason provided';

    final productName = adjustment.product?.name ?? 'Unknown Product';
    final createdByName = adjustment.createdBy?.name ?? 'Unknown User';
