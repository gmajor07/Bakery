...
class _PurchaseOrdersScreenState extends ConsumerState<PurchaseOrdersScreen> {
  ...
  DateTimeRange? _getDateRange(QuickDateFilter filter) {
    ...
    switch (filter) {
      case QuickDateFilter.all: return null;
      case QuickDateFilter.today: return DateTimeRange(start: today, end: endOfToday);
      ...
      case QuickDateFilter.last7Days:
        return DateTimeRange(start: today.subtract(const Duration(days: 6)), end: endOfToday);
      ...
      case QuickDateFilter.custom: return customDateRange;
    }
  }

  String _getFilterName(QuickDateFilter filter) {
    switch (filter) {
      case QuickDateFilter.all: return 'All Time';
      case QuickDateFilter.today: return 'Today';
      case QuickDateFilter.yesterday: return 'Yesterday';
      case QuickDateFilter.last7Days: return 'Last 7 Days';
      case QuickDateFilter.thisMonth: return 'This Month';
      case QuickDateFilter.lastMonth: return 'Last Month';
  ...
  List<PurchaseOrder> _applyFilters(List<PurchaseOrder> orders) {
    ...
    return orders.where((order) {
      ...
      if (selectedRange != null) {
        ...
        matchesDate = orderDate.isAfter(rangeStart.subtract(const Duration(seconds: 1))) &&
  ...
  List<PurchaseOrder> _applyPagination(List<PurchaseOrder> orders, PaginationState pagination) {
  ...
  Widget build(BuildContext context) {
    ...
    final hasFilters = ref.watch(selectedPurchaseDateRangeProvider) != null ||
    ...
    return Scaffold(
              ...
              Expanded(
                child: asyncOrders.when(
                  data: (allOrders) {
                    ...
                    final paginated = _applyPagination(filtered, paginationState);
                    return _buildOrdersList(paginated, filtered.length, paginationState);
                  },
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          final result = await Navigator.push(context, MaterialPageRoute(builder: (_) => const CreatePurchaseOrderScreen()));
  ...
  Widget _buildStatusFilter(String? selected) {
    ...
    return DropdownButtonFormField<String?>(
      ...
      items: statuses.map((s) => DropdownMenuItem(value: s, child: Text(s == null ? "All Status" : _capitalizeFirstLetter(s)))).toList(),
      onChanged: (value) {
  ...
  Widget _buildOrdersList(List<PurchaseOrder> orders, int total, PaginationState pagination) {
          ...
          child: RefreshIndicator(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) => PurchaseOrderCard(
                order: orders[index],
                onTap: () async {
                  final result = await Navigator.push(context, MaterialPageRoute(builder: (_) => PurchaseOrderDetailScreen(order: orders[index])));
                  if (result == true) _refreshData(); // Refresh if detail screen causes an update
                },
  ...
  Widget _buildPaginationControls(int total, PaginationState pagination) {
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ElevatedButton(
            onPressed: pagination.currentPage > 1 ? () => ref.read(purchasePaginationProvider.notifier).previousPage() : null,
            child: const Text("Prev"),
          ),
          Padding(padding: const EdgeInsets.symmetric(horizontal: 16), child: Text("${pagination.currentPage} / $totalPages")),
          ElevatedButton(
            onPressed: pagination.currentPage < totalPages ? () => ref.read(purchasePaginationProvider.notifier).nextPage() : null,
  ...
  Widget _buildEmptyState() => const Center(child: Text("No orders found for this selection."));
}
    ...
    return Card(
      ...
      child: InkWell(
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              CircleAvatar(backgroundColor: statusColor.withOpacity(0.1), child: Icon(Icons.shopping_bag, color: statusColor)),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(order.supplier.name, style: TextStyle(color: Colors.grey.shade700)),
              ...
              Column(
                ...
                children: [
                  Text(amount, style: TextStyle(fontWeight: FontWeight.bold, color: statusColor)),
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: statusColor,
                      borderRadius: BorderRadius.circular(4), // Sharp rectangle edges
                    ),
                    child: Text(
